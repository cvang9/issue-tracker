<template>
  <div class="login-box">
    <div class="login-box-formbox">
      <div
        v-if="invalidCreds"
        class="my-6 flex w-full border-l-6 border-[#F87171] bg-[#F87171] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9"
      >
        <div
        ...
        <div class="w-full">
          ...
          <ul>
            <li class="leading-relaxed text-[#CD5D5D]">Please check your credentials again</li>
          </ul>
        </div>
      </div>
      ...
      <div class="login-box-login">
        <p class="text-xl mx-3">Login {{ role.charAt(0).toUpperCase() + role.slice(1) }}</p>
        <!-- <p v-if="invalidCreds" class="text-md mx-3" style="color: red" >Invalid Credentials</p> -->
        ...
        <form>
          ...
          <button
            ...
            v-on:click.prevent="submitButtonHandler"
          >
            Login
          </button>
          <button
            ...
            disabled
            type="button"
            class="flex justify-center rounded bg-primary py-2 px-6 font-medium text-gray hover:bg-opacity-90 inline-flex items-center"
          >
            <svg
              aria-hidden="true"
              role="status"
  ...
  setup() {
    ...
    onMounted(() => {
      if (localStorage.getItem('token') !== null) {
        if (localStorage.getItem('role') === 'resolver') {
          const id = localStorage.getItem('id')
          route.push(`/resolver/${id}`)
        }
      }
    })
